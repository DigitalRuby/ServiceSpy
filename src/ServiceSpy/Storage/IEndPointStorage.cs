...
public interface IEndPointStorage
{
    ...
    Task<EndPoints?> GetAsync(Guid id, CancellationToken cancelToken = default);
    ...
    Task<IReadOnlyDictionary<EndPoint, EndPoint?>?> UpsertAsync(Guid id, IEnumerable<EndPoint> endPoints, CancellationToken cancelToken = default);

    /// <summary>
    /// Delete the service end point for the specified service name
    /// </summary>
    /// <param name="id">Service id</param>
    /// <param name="endPoints">End points to delete</param>
    /// <param name="cancelToken">Cancel token</param>
    ...
    Task<(bool, bool)> DeleteAsync(Guid id, IEnumerable<EndPoint> endPoints, CancellationToken cancelToken = default);
    ...
    Task<IReadOnlyCollection<EndPoint>?> DeleteAllAsync(Guid id, CancellationToken cancelToken = default);
}
